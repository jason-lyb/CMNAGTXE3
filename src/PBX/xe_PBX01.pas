...
{			iRow := frm_PBX.cxGridPBX3.DataController.FindRecordIndexByText(0, 1, Trim(edt_1.Text), False, True, True);
			...
			frm_PBX.cxGridPBX3.DataController.Values[iRow, 1] := edt_1.Text;   //호분배 그룹
			frm_PBX.cxGridPBX3.DataController.Values[iRow, 2] := StrToCall(edt_12.Text); //대표번호
			frm_PBX.cxGridPBX3.DataController.Values[iRow, 3] := StrToCall(edt_2.Text); //착신번호
			frm_PBX.cxGridPBX3.DataController.Values[iRow, 4] := edt_3.Text;   //서비스명
			frm_PBX.cxGridPBX3.DataController.Values[iRow, 5] := edt_4.Text;   //명의자/통신사
			frm_PBX.cxGridPBX3.DataController.Values[iRow, 6] := edt_5.Text;   //안내문구
			if chk_Tell.checked = True     then frm_PBX.cxGridPBX3.DataController.Values[iRow,11] := '사용'
																		 else frm_PBX.cxGridPBX3.DataController.Values[iRow,11] := '';    //콜마너 OEM 연동 유무 N : 사용안함 Y : 사용
			if chk_TellTak.checked = True  then frm_PBX.cxGridPBX3.DataController.Values[iRow,12] := '사용'
																		 else frm_PBX.cxGridPBX3.DataController.Values[iRow,12] := '';    //콜마너 탁송 연동 유무 N : 사용안함 Y : 사용
			if chk_App.checked = True      then frm_PBX.cxGridPBX3.DataController.Values[iRow,28] := '사용'
																		 else frm_PBX.cxGridPBX3.DataController.Values[iRow,28] := '미사용';//어플연동
			if chk_Logi.checked = True     then frm_PBX.cxGridPBX3.DataController.Values[iRow,29] := '사용'
																		 else frm_PBX.cxGridPBX3.DataController.Values[iRow,29] := '';		//로지 연동 유무 N : 사용안함 Y : 사용
			if chk_CallMart.checked = True then frm_PBX.cxGridPBX3.DataController.Values[iRow,30] := '사용'
																		 else frm_PBX.cxGridPBX3.DataController.Values[iRow,30] := '';		//콜마트 연동 유무 N : 사용안함 Y : 사용
			if chk_ICon.checked = True     then frm_PBX.cxGridPBX3.DataController.Values[iRow,31] := '사용'
																		 else frm_PBX.cxGridPBX3.DataController.Values[iRow,31] := '';		//아이콘 연동 유무 N : 사용안함 Y : 사용
			if chk_G365.checked = True     then frm_PBX.cxGridPBX3.DataController.Values[iRow,32] := '사용'
																		 else frm_PBX.cxGridPBX3.DataController.Values[iRow,32] := '';		//G365 유무 N : 사용안함 Y : 사용
			if chk_InSung.checked = True   then frm_PBX.cxGridPBX3.DataController.Values[iRow,33] := '사용'
																		 else frm_PBX.cxGridPBX3.DataController.Values[iRow,33] := '';		//인성 연동 유무 N : 사용안함 Y : 사용
		
			frm_PBX.cxGridPBX3.DataController.Values[iRow,13] := cb_2.text;                                 //CTI-Q 대기멘트
			frm_PBX.cxGridPBX3.DataController.Values[iRow,14] := cb_3.text;                                 //VIP CTI-Q 대기멘트
			if cb_6.ItemIndex = 0 then
				frm_PBX.cxGridPBX3.DataController.Values[iRow, 8] := '사용안함'       else
			if cb_6.ItemIndex = 1 then
				frm_PBX.cxGridPBX3.DataController.Values[iRow, 8] := '개별착신사용'   else
			if cb_6.ItemIndex = 2 then
				frm_PBX.cxGridPBX3.DataController.Values[iRow, 8] := '지능형착신사용' else 
				frm_PBX.cxGridPBX3.DataController.Values[iRow, 8] := '';                      //개별착신/지능형착신 사용 유무 
			frm_PBX.cxGridPBX3.DataController.Values[iRow, 7] := StrToCall(edt_6.Text);     //개별착신 / 지능형 착신 번호 1
			frm_PBX.cxGridPBX3.DataController.Values[iRow, 9] := edt_7.Text;                //개별착신 / 지능형 착신 시작시간
			frm_PBX.cxGridPBX3.DataController.Values[iRow,10] := edt_8.Text;                //개별착신 / 지능형 착신 종료시간

			if cb_7.ItemIndex = 0 then frm_PBX.cxGridPBX3.DataController.Values[iRow,17] := '사용안함' 
														else frm_PBX.cxGridPBX3.DataController.Values[iRow,17] := '사용'    ;//개별 필터링 사용유무 N : 사용안함 Y : 사용
			frm_PBX.cxGridPBX3.DataController.Values[iRow,18] := cb_8.Text;                            //개별 필터링 멘트 명
			frm_PBX.cxGridPBX3.DataController.Values[iRow,19] := edt_9.Text;                           //개별 필터 시작 시간
			frm_PBX.cxGridPBX3.DataController.Values[iRow,20] := edt_10.Text;                          //개별 필터 종료 시간

			if cb_9.ItemIndex = 0 then frm_PBX.cxGridPBX3.DataController.Values[iRow,21] := '사용안함' 
														else frm_PBX.cxGridPBX3.DataController.Values[iRow,21] := '사용'    ;//080 수신거부 사용유무 N : 사용안함 Y : 사용
			frm_PBX.cxGridPBX3.DataController.Values[iRow,26] := cb_1.Text;                            //080 수신거부 파일 명
	
			if cb_14.ItemIndex = 0 then frm_PBX.cxGridPBX3.DataController.Values[iRow,27] := '1' 
														 else frm_PBX.cxGridPBX3.DataController.Values[iRow,27] := '2'    ;//호분배 순위 ( 1 or 2 순위)
}		end;   
