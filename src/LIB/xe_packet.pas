...
begin
  ...
  try
    if dm.SendSock(AQuery, StrList, AErrCode, False) then
    begin
      AData := StrList[0];
...
begin
  Result := False;
  try
    if dm.SendSock(AQuery, AData, AErrCode, False) then
    begin
      xdom := ComsDomDocument.Create;
      try
...
begin
  ...
  Result := StringReplace(Result, 'UserPWValue', En_Coding(GT_USERIF.PW),  [rfReplaceAll]);
...
begin
  ...
  Result := StringReplace(Result, 'UserPWValue', GT_USERIF.PW, [rfReplaceAll]);
  Result := StringReplace(Result, 'LocalIPString', GT_USERIF.IP, [rfReplaceAll]);
