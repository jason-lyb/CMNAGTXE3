...
uses
  ...
  Vcl.Controls, System.Classes, Winapi.ShellAPI, System.Win.Registry, dxCore,
  //---------------------
  Forms, Messages, SysUtils, Variants, Graphics, Dialogs, MSXML2_TLB, ComObj, IniFiles, IDGlobal, Imm, System.StrUtils, WinSvc,
  dxSkinOffice2010Silver, dxSkinSharp, dxBarBuiltInMenu, cxStyles, cxCustomData, cxFilter, cxData, cxDataStorage, cxNavigator, dxDateRanges, Data.DB,
  cxDBData, cxGridLevel, cxGridCustomTableView, cxGridTableView, cxGridDBTableView, cxClasses, cxGridCustomView, cxGrid, cxPC,
...
type
  TFrm_Login = class(TForm)
    ...
    procedure cxGrid_LogAssSCellClick(Sender: TcxCustomGridTableView; ACellViewInfo: TcxGridTableDataCellViewInfo; AButton: TMouseButton;
...
procedure TFrm_Login.cxGrid_LogAssSCellClick(Sender: TcxCustomGridTableView; ACellViewInfo: TcxGridTableDataCellViewInfo; AButton: TMouseButton;
  AShift: TShiftState; var AHandled: Boolean);
begin
...
var
  sTemp: AnsiString;
  bCheckID, bCheckPass, bCheckConfig : Boolean;
  EnvFile: TIniFile;
  ...
  try
    ...
    begin
			...
			if GS_PRJ_AREA = 'S' then
				...
				sTemp := GetRegistry(HKEY_LOCAL_MACHINE, 'SYSTEM\WLKXE3JI', 'LOGID');
      ...
      begin
		...
		begin
			...
			if GS_PRJ_AREA = 'S' then
				...
				sTemp := GetRegistry(HKEY_LOCAL_MACHINE, 'SYSTEM\WLKXE3JI', 'PASSWD');
...
/////////////////로그인 성공 후 ID 비번 저장 20190819 KHS///////////////////////////////////
			...
			begin
				...
				if GS_PRJ_AREA = 'S' then
					SetRegistry(HKEY_LOCAL_MACHINE, 'SYSTEM\WLKXE3', 'LOGID', sLogID)
				else
					SetRegistry(HKEY_LOCAL_MACHINE, 'SYSTEM\WLKXE3JI', 'LOGID', sLogID);
		  end;
			...
			begin
				...
				if GS_PRJ_AREA = 'S' then
					SetRegistry(HKEY_LOCAL_MACHINE, 'SYSTEM\WLKXE3', 'PASSWD', sPass)
				else
					SetRegistry(HKEY_LOCAL_MACHINE, 'SYSTEM\WLKXE3JI', 'PASSWD', sPass);
			end;
...
/////////////////로그인 성공 후 ID 비번 저장 20190819 KHS///////////////////////////////////
      ...
      begin
        ...
        with GT_USERIF do
          ...
          ShareNo := lst_Result.item[0].attributes.getNamedItem('ShareNo').Text;
          if GS_PRJ_AREA = 'O' then
          begin
...
//				GT_USEKM := IsKMUSEHead(GT_USERIF.HD);
      ...
      // hjf 확인필요
        ...
        if GT_USERIF.HD = 'A1531' then
          SetRegistry(HKEY_LOCAL_MACHINE, 'SYSTEM\WLKXE3', 'PASSWD', '');
...
//						1101: TCK_USER_PER.STT_Day := '1';
            1102: TCK_USER_PER.STT_Keynumber := '1';                // 통계>대표번호별통계
            1103: TCK_USER_PER.STT_Worker := '1';                   // 통계>기사별통계
//            1104: TCK_USER_PER.STT_User := '1';                     // 통계>사용자별
//            1105: TCK_USER_PER.STT_Customer := '1';                 // 통계>고객별
//            1106: TCK_USER_PER.STT_NewCustRate := '1';              // 통계>신규고객율
//            1107: TCK_USER_PER.STT_Cancel := '1';                   // 통계>취소
//            1108: TCK_USER_PER.STT_Owner := '1';
//            1109: TCK_USER_PER.STT_Share := '1';                    // 통계>공유
//						1110: TCK_USER_PER.STT_Local := '1';                    // 통계>지역별
...
//					GS_NAVERPOI_SEL      := True ;// ln_envfile.ReadBool('SearchSet', 'SEARCH_NAVERPOI_SEL', True);	 무조건 NN으로 팀장님지시 20191203 KHS
          ...
          GS_BRLISTVIEWMODE := ln_envfile.ReadBool('COUNSEL', 'BRLISTVIEWMODE', False);   // 2015.11.13 좌측지사리스트 본사단위 표시 여부
					GS_JON54AutoRunNew := ln_envfile.ReadBool('COUNSEL', 'JON54AutoRunNew', True);   // 2016.06.20 프로그램 실행시 통화��리 자동실행
