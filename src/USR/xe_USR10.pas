...
uses
  ...
  dxSkinsCore, dxSkinOffice2010Silver;
...
Var	sTemp, sTempLog, sAge: string;
...
begin
  ...
  begin
    GMessagebox('상담원 권한은 상담원 직원만 등록/수정 가능합니다.', CDMSI);
    cxcbGrad.SetFocus;
    Exit;
  end;
  ...
  begin
    GMessagebox('직원 그룹을 선택하세요.', CDMSI);
    Exit;
  end;
  ...
  begin
		...
		begin
			...
			begin
				...
				begin
					GMessagebox('기사등록 불가한(임시거주 제2외국인, 외국인,, 등등) 주민번호를 입력하였습니다.', CDMSI);
  				Exit;
				end;
			end else
      begin
        GMessagebox('주민등록 번호를 잘못 입력하였습니다.', CDMSI);
				Exit;
      end;
		end;  }
	end else
	begin
		GMessagebox('생년월일을 잘못 입력하였습니다.', CDMSI);
  	Exit;
	end;
...
{	if btnNCert.Enabled then
  ...
  begin
    ...
    begin
      GMessagebox('중복체크 버튼으로 아이디 중복검사를 하세요', CDMSI);
    end else
    begin
      if Length(cxedId.Text) < 6 then
      begin
        GMessagebox('아이디는 6자이상 입력하셔야 합니다.', CDMSI);
        exit;
      end;
      proc_Manage_Save(0);
    end;
...
var
	sTemp, sParam, sBrNo, sHdNo, sLevel, sId, sPwd, sName, sUserStatus, sHp, sTel,
	sSNum, sRegDate,
	sAddr, sMemo: string;
	i, iCnt: Integer;
	ErrCode: integer;
...
begin
	...
	begin
		GMessagebox(Format('직원 저장 중 오류발생'#13#10'[%d]%s', [ErrCode, ErrMsg]), CDMSI);
		Exit;
	end;
	if iType = 1 then
	begin
...
begin
  ...
  begin
    try
      ...
      if tmpStr<>'Y' then
      begin
        GMessagebox('콜센터인 지사에서만 상담원을 등록 할 수 있습니다.', CDMSI);
        cxcbGrad.ItemIndex := -1;
      end;
    except
      GMessagebox('콜센터유무조회중 오류가 발생했습니다.',CDMSE);
