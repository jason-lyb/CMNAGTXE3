...
uses
  ...
  IdComponent, IdTCPConnection, IdTCPClient, IdHTTP, dxSkinOffice2016Colorful, dxDateRanges;

type
  Tfrm_AIC23 = class(TForm)
    pnl_main: TPanel;
    pnlTitle: TPanel;
    cxLblActive: TLabel;
    BtnClose: TcxButton;
    ...
    cxGridAI2_List2Column7: TcxGridDBColumn;
    ...
    procedure cxGridAI2_List2Column2CustomDrawCell(
...
procedure Tfrm_AIC23.BtnCloseClick(Sender: TObject);
begin
	close;
end;

procedure Tfrm_AIC23.btnExitClick(Sender: TObject);
begin
...
procedure Tfrm_AIC23.cxGridAI2_List2CellClick(Sender: TcxCustomGridTableView;
	ACellViewInfo: TcxGridTableDataCellViewInfo; AButton: TMouseButton;
  AShift: TShiftState; var AHandled: Boolean);
var 	XmlData, Param, ErrMsg, ls_MSG_Err : string;
	...
	tmpCnt : Integer;
begin
	SetDebugeWrite('Tfrm_AIC23.cxGridAI2_List2CellClick');
	Try
        ...
        begin
				...
				try
					...
					// 순번 | 처리일시 | SEQ | AI서버(음성인식시스템 질문내역) | 고객답변내역 | ppn번호 | 저장일시
						...
						cxGridAI2_List3.DataController.Values[iRow, 7] := '';
...
procedure Tfrm_AIC23.cxGridAI2_List2Column2CustomDrawCell(
  Sender: TcxCustomGridTableView; ACanvas: TcxCanvas;
  AViewInfo: TcxGridTableDataCellViewInfo; var ADone: Boolean);
Var
...
{	AView := Sender as TcxGridDBTableView;
	...
	Inx := AView.GetColumnByFieldName('전체녹취청취').Index;
	if Trim(AViewInfo.GridRecord.Values[Inx]) = 'y'   then 
		TcxButtonEditProperties(cxGridAI2_List2Column2.Properties).Buttons.Items[0].Enabled := True
	else
		TcxButtonEditProperties(cxGridAI2_List2Column2.Properties).Buttons.Items[0].Enabled := False;  }
end;

procedure Tfrm_AIC23.cxGridAI2_List2Column2PropertiesButtonClick(
...
procedure Tfrm_AIC23.cxGridAI2_List2Column5PropertiesButtonClick(
  Sender: TObject; AButtonIndex: Integer);
Var iRow, iSpeak, iDx : Integer;
	...
	sFileNm, sYMD : String;
begin
	SetDebugeWrite('Tfrm_AIC23.cxGridAI2_List2Column5PropertiesButtonClick');
	Try
		iRow := cxGridAI2_List2.DataController.FocusedRecordIndex;
		if iRow  = -1 then Exit;
		iCol := cxGridAI2_List2.GetColumnByFieldName('ppn').Index;
		...
		sUrl := 'http://211.56.137.212:9425/ss15_rec/' + sFileNm;
		sFileNm := sYMD + '-' + cxGridAI2_List2.DataController.Values[iRow, iCol] + '.wav';
...
procedure Tfrm_AIC23.cxGridAI2_List3Column2PropertiesButtonClick(
  Sender: TObject; AButtonIndex: Integer);
Var iRow, iSpeak, iDx : Integer;
	sSpeak, sUrl, sTmp, sFileName, sIP : string;
	fsData : TFileStream;
	...
	sFileNm, sYMD : String;
begin
	SetDebugeWrite('Tfrm_AIC23.cxGridAI2_List3Column2PropertiesButtonClick');
	Try
...
procedure Tfrm_AIC23.cxGridDBColumn6PropertiesButtonClick(Sender: TObject;
	AButtonIndex: Integer);
Var iRow, iCol, iDate : Integer;
		sFileNm, sYMD : String;
begin
...
procedure Tfrm_AIC23.FormActivate(Sender: TObject);
begin
	cxLblActive.Visible := True;
end;

procedure Tfrm_AIC23.FormClose(Sender: TObject; var Action: TCloseAction);
begin
	Action := caFree;
end;

procedure Tfrm_AIC23.FormCreate(Sender: TObject);
var i : integer;
...
procedure Tfrm_AIC23.FormDeactivate(Sender: TObject);
begin
	cxLblActive.Visible := False;
end;

procedure Tfrm_AIC23.FormDestroy(Sender: TObject);
begin
	Frm_AIC23 := Nil;

end;

function Tfrm_AIC23.func_FileDown(ALink: string): Boolean;
var sSpeak, sUrl, sTmp, sFileName : string;
	fsData : TFileStream;
begin
	Result := False;
	Try
...
function Tfrm_AIC23.func_FileStream(AFileNm, AUrl: String): Boolean;
...
Var iRow, iSpeak, iDx : Integer;
	...
	sFileNm, sYMD : String;
begin
	SetDebugeWrite('Tfrm_AIC23.func_FileStream');
	Try
		Result := False;
		sUrl := AUrl;
		sFileNm := AFileNm;
...
procedure Tfrm_AIC23.pnlTitleMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  ReleaseCapture;
...
procedure Tfrm_AIC23.pPlay(sFile: String);
...
//		grpPlayer.Visible := True;
		...
		if Dm.gsTestPORTYN = 'Y' then
			sUrl := 'http://123.142.233.115:9425/ss15_rec/' + sFile
		else
			sUrl := 'http://211.56.137.212:9425/ss15_rec/' + sFile;
...
procedure Tfrm_AIC23.proc_Get_List(AList: string);
var 	XmlData, Param, ErrMsg, ls_MSG_Err : string;
	...
	ls_Rcrd : TStringList;
begin
	SetDebugeWrite('Tfrm_AIC23.proc_Get_List');
	Try
		XmlData := AList;
		xdom := ComsDomDocument.Create;
		cxGridAI2_List2.DataController.SetRecordCount(0);
				...
				begin
					...
					try
						...
						// 순번 | 통화시작시간 | 통화종료시간 | 통화구분 | 차감캐쉬 | 녹취내역
							...
							cxGridAI2_List2.DataController.Values[iRow, 9] := '';
...
procedure Tfrm_AIC23.proc_Init;
var i : integer;
begin
	cxGridAI2_List2.DataController.SetRecordCount(0);
	for i := 0 to cxGridAI2_List2.ColumnCount - 1 do
